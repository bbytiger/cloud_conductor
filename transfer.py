...
def simulate_S3_to_GCP():
    ...
    aws.create_bucket(src_bucket_name, source_region, {})
    src_bucket = s3.Bucket(src_bucket_name)
    aws.upload_data(src_key, src_bucket)
    
    # create dst gcp bucket and perform transfer
    storage_client = storage.Client(project='grounded-datum-367811')
    gc_bucket = googlecloud.create_bucket(storage_client, dst_bucket_name, destination_region)
...
def simulate_GCP_to_S3():
    ...
    storage_client = storage.Client(project='grounded-datum-367811')
    src_bucket = googlecloud.create_bucket(storage_client, src_bucket_name, source_region)
    googlecloud.upload_data(src_key, src_bucket)

    # create dst aws bucket and perform transfer
    aws.create_bucket(dst_bucket_name, destination_region, {})
