...
from serverutils import ServerRSAKeyManager
import typelayer
...
class CliManager:
    def __init__(self, parent_dir):
        ...
        self.gcp_config = self.load_gcp_config()
        self.rsa_key_manager = ServerRSAKeyManager()
    ...
    def load(self):
        ...
        config = self.path / "config"
        if config.exists():
            return CliUtils.read_pyobj(config)
        else:
            return []

    def write_gcp_config(self):
        config = self.path / "gcpconfig"
        if self.gcp_config:
            CliUtils.write_pyobj(self.gcp_config, config)

    def load_gcp_config(self):
        config = self.path / "gcpconfig"
        if config.exists():
            return CliUtils.read_pyobj(config)
        else:
...
def setup(ctx, csp):
    def onboard_aws():
        ...
        subprocess.check_output(
            "./scripts/aws.sh", stderr=subprocess.STDOUT, shell=True
        )
    ...
    def onboard_gcp():
        ...
        subprocess.check_output(
            "./scripts/gcp.sh", stderr=subprocess.STDOUT, shell=True
        )
        click.echo("Successfully installed GCP SDK.")
        click.echo(
            "Retrieve service account JSON key file from GCP IAM Dashboard",
        )
        ...
        subprocess.run([activate_service_account], capture_output=True)
        project_id = click.prompt("Enter project id:")
        retrieve_api_key = f"{gcp_sdk_root} auth application-default print-access-token"
        api_key = subprocess.run(
            [retrieve_api_key], capture_output=True
        ).stdout.decode()
        gcp_config = {"api_key": api_key, "project_id": project_id}
        ctx.object["MANAGER"].gcp_config = gcp_config
        ctx.object["MANAGER"].write_gcp_config()
...
def create_pipeline(ctx, name, stagecount):
    def create_stage(pipeline_name, index):
        ...
        dst_path = CliUtils.prompt_until_success(
            ...
            lambda user_inp: None
            if user_inp == ""
            else CliUtils.validate_path(user_inp),
        )
        location_constraint = CliUtils.prompt_until_success(
            "Enter location constraint: (RETURN if none)",
            lambda user_inp: (None if user_inp == "" else user_inp),
        )
        ...
        return typelayer.Stage(
            ctx_manager=ctx.object["MANAGER"],
    ...
    pipeline = typelayer.Pipeline(
        stagecount, name, initial_location, datetime.datetime.now()
    )
